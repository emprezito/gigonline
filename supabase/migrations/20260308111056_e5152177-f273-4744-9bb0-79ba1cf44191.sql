...
-- by dropping and re-creating each one.
-- Also fix lesson_progress UPDATE to include WITH CHECK.
-- ============================================================
...
CREATE POLICY "Users can update own progress" ON public.lesson_progress FOR UPDATE TO authenticated
  USING (auth.uid() = user_id)
...
CREATE POLICY "Affiliates can request pending payouts" ON public.payouts FOR INSERT TO authenticated
  WITH CHECK ((status = 'pending') AND (approved_at IS NULL) AND (completed_at IS NULL) AND (transfer_reference IS NULL) AND (EXISTS (
    SELECT 1 FROM affiliates WHERE affiliates.id = payouts.affiliate_id AND affiliates.user_id = auth.uid() AND affiliates.approved = true AND affiliates.enabled = true
  )));
